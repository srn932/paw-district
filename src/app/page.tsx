...
export const metadata = pageMetadata("Paw District | Pet Boarding, Grooming & Dog Training in Chennai", "Thoughtful pet boarding, professional grooming and practical dog training in one happy district", "/");
  ...
  return (
    <>
      ...
      <section className="container-shell pb-16 pt-32 md:pb-24 md:pt-40 lg:pt-44">
        <div className="grid items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
          <div className="relative z-10">
            ...
            <Reveal delay={.12}><p className="body-lg mt-7 max-w-xl">From comfortable stays and supervised play to fresh trims and practical training, Paw District brings thoughtful pet care together in Chennai.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><ButtonLink href="/services/boarding">Explore boarding</ButtonLink><ButtonLink href="/visit" variant="outline">Visit the District</ButtonLink></div><p className="mt-8 text-xs font-bold uppercase tracking-[.12em] text-muted">Boarding <span className="mx-2 text-leaf">·</span> Grooming <span className="mx-2 text-leaf">·</span> Training</p></Reveal>
          </div>
      ...
      <section className="bg-cream section-pad">
        <div className="container-shell"><Reveal><SectionHeading eyebrow="02 · What we do" title="Three ways we help." copy="Boarding, grooming and training—one familiar team that learns the details that make your pet, your pet." /></Reveal><div className="mt-14 grid gap-6 lg:grid-cols-3">{featuredServices.map((service, index) => <Reveal key={service.slug} delay={index * .06}><ServiceCard service={service} index={index} /></Reveal>)}</div></div>
      </section>

      <section className="container-shell section-pad"><Reveal><SectionHeading eyebrow="03 · Easy does it" title="Three steps to a happier day." /></Reveal><div className="mt-14 grid gap-5 lg:grid-cols-3">{[["01","Tell us about them","Age, temperament, routine and what you would like help with."],["02","Choose their focus","Plan a boarding stay, grooming session or training conversation."],["03","Let the tail wag","We take it from here—and keep you in the loop."]].map(([number,title,copy], index) => <Reveal key={number} delay={index*.08} className="group rounded-4xl border border-ink/10 p-7 transition hover:bg-mint md:p-9"><span className="text-xs font-extrabold tracking-[.18em] text-leaf">STEP {number}</span><div className="my-12 h-px bg-ink/10 transition group-hover:bg-forest/25" /><h3 className="subhead">{title}</h3><p className="mt-4 leading-7 text-muted">{copy}</p></Reveal>)}</div></section>
